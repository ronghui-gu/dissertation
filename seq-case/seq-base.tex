...
\end{mathpar}
Each entry of allocation table is a tuple
$(\code{is\_alloc}, \code{ref}, \code{owner}, \code{status})$,
where $\code{is\_alloc}:\code{bool}$ indicating
...
and set the page status to be $\code{hidden}$ (\cf Figure~\ref{fig:seq:pages}).
\begin{mathpar}
\inferrule{
...
\subsection{The shared memory management} provides a protocol to share physical
