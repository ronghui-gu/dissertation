...
\paragraph{Initialization} The initialization primitive 
$\code{preinit}$
...
\\
\code{addr\_trans}(a,v) = \code{Some}\ v'
}{ \Gamma \vdash\code{PreInit}.\mat.load(v; \rho, m, a)  = (m(v'); \rho, m, a)}
\and
\inferrule{ a.\code{CR0} = \code{true}
\\
\code{addr\_trans}(a,v) = \code{None}\\
...
It is state as a layer invariant of \code{PreInit}.
...
\begin{mathpar}
\inferrule{
q = a.\code{cn}(\mathit{tid}).\code{quota} \ge 1 \\
...
a' = a \set{\code{AT},\mathit{freei}: (\true, 1,\mathit{tid})}
\set{\code{cn}.\code{quota},\mathit{tid}: q - 1}
}{
\Gamma  \vdash  \spec_{\code{palloc}}([\mathit{tid}],m, a, \mathit{freei}, m, a')
}
\and
\inferrule{
(a.\code{cn}(\mathit{tid}).\code{quota} < 1)
\vee
(\forall 0 \le i < a.\code{nps}, a.\code{AT}(i).\code{is\_alloc} = \true)
}{
\Gamma  \vdash  \spec_{\code{palloc}}([\mathit{tid}],m, a, \code{ERROR}, m, a)
}
\end{mathpar}
...
2) $\forall 0 \le i \le 2^{18},  a.\code{AT}(i).\code{is\_alloc} = \true$;
