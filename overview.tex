...
Our new \CTOS\ architecture aims to address all these concerns
and also tackle all four challenges described in Chapter~\ref{chap-intro}.
The key technologies are abstraction layers and deep specifications.
...
\caption{Certified sequential OS kernels: what to prove?}
\hrulefill
%\rule[0in]{\columnwidth}{.15mm}
\label{fig:mainthm}
\end{figure}

Under \CTOS, building a new certified sequential kernel (or experimenting with a new
design) is just a matter of composing a collection of certified
sequential layers, developed in a variant of C (called ClightX) or assembly.
...
In Figure~\ref{fig:mainthm}\ronghui{$\oplus$}, we use x86 to denote an assembly machine in the sequential setting
and {$\sem{\rm{}x86}{\cdot}$} for its whole-machine semantics.
Suppose we load such a machine with the \mCTOS\ sequential kernel $K$ (in
...
specification of $K$ (in terms of system calls). We use $\sem{\rm\mCTOS}{\cdot}$ to denote its
...
\caption{Overview of the sequential \CTOS\ architecture}
\hrulefill
...
In a concurrent layer, the thread-private abstract states 
are also refined from the concrete
thread-local in-memory data and are manipulated by abstract primitives,
which is similar to the sequential abstraction layer. 
The data shared by multiple threads are kept track of by a shared
global log. Each operation to the shared data is
recorded as an observable event,  which will be appended to the end of the log. For each piece (or CompCert memory block) of the shared data, we define a {\em replay} function
that can reconstruct the current shared state from the global
log. Thus, all the shared data can be represented as a single sequence of
...
we have to reason about all possible scheduler strategies.
Given a strategy $\strat{hs}$, if running $P\oplus{}M$ on
machine $L$ produces a global log $l$, we must find
...
demonstrate that the implementation of $\acq$ meets its specification.
