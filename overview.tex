...
systems in a cost-effective way. We enumerate a few important
...
to construct certified OS kernels with deep specifications.
...
$P\oplus{}M_1$ (\ie, $P$ linked with $M_1$) should lead to the same
...
A {\em certified abstraction layer} is a language-based module construct that
consists of a triple $\layer{L_1}{M}{L_2}$ plus a mechanized proof
object the predicate $\ltyp{L_1}{R}{M}{L_2}$,
...
Thus, $L_2$ is
a deep
specification of a module (\eg, $M$), which  captures
...
In Figure~\ref{fig:mainthm}, we use x86 to denote an assembly machine
...
overlay (i.e., $L_i$) but it can only call the primitives in its
underlay ($L_{i-1}$).  Using vertical composition~\cite{dscal15}, from
...
efficient assembly implementation (i.e., $K$).
